function resGroup = modularity_group_Newman(Ci, result_path, threshold, type, doRandom, nRandom, randomFunction, groups_label, subj_agree)   
   agree = agreement(Ci);
   agree = agree ./ length(Ci);
   tau = 0;
   reps = 1000;
   [C, Q] = consensus_adapted_newman(agree, tau, reps);
   ...
   if doRandom > 0
       if ~strcmp(randomFunction, 'null_model_und_sign')
           uiwait(msgbox('Random Networks for group modularity measures are always created using "null_model_und_sign"! ','Success','modal'));
       end
       ...
       save([result_path filesep 'GraphVars' filesep groups_label '_consensus_Newman_perm_C_' num2str(threshold*10) '_' num2str(type) '.mat'], 'C_iter', '-v7.3');
   end
  
   for i_res = 1:length(resultslist)
       Result = resultslist{i_res};
       save([result_path filesep 'GroupCommunity' filesep groups_label '_modularity_group_Newman' '_' namelist{i_res} '_' num2str(threshold*10) '_' num2str(type) '.mat'], 'Result', '-v7.3');
   end
